...
echo -e "${GREEN}=== INSTALADOR TODO-EN-UNO SISTEMA ISP (CORREGIDO) ===${NC}"
...
# 2. CONFIGURAR BD
echo -e "${GREEN}[2/5] Configurando Base de Datos...${NC}"
...
# 3. ESTRUCTURA SQL
echo -e "${GREEN}[3/5] Creando tablas...${NC}"
mysql ${DB_NAME} <<EOF
...
# 4. GESTIÓN DE ARCHIVOS (Aquí estaba el error)
echo -e "${GREEN}[4/5] Verificando archivos web...${NC}"
cd /var/www/html

read -p "Introduce URL del Repo GitHub (Presiona ENTER si ya subiste los archivos manualmente): " REPO_URL

if [ ! -z "$REPO_URL" ]; then
    # SOLO borramos si el usuario eligió descargar desde GitHub
    rm -f index.html 
    ...
    echo -e "${YELLOW}Modo Manual: No se han tocado tus archivos existentes.${NC}"
fi

# 5. CONEXIÓN API (Vital)
if [ -f "api.php" ]; then
    sed -i "s/UnaClave_MuyDificil_99\\$/${DB_PASS}/g" api.php
    echo "Clave de API actualizada."
else
    echo -e "${RED}¡ALERTA! No se encontró api.php. (Si los subiste manual, verifica el nombre).${NC}"
fi
